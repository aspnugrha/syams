...
class AuthAdminController extends Controller
{
    ...
    public function forgotPasswordProcess(Request $request){
        ...
        try {
            ...
            $url = route('paneladmin.set-new-password', ['email' => $email_encode, 'code' => $activation_code_encode]);

            $company_profile = CompanyProfile::first();
            Mail::to($request->email)->send(new ForgotPasswordEmail($user, $url, $company_profile));
    ...
    public function registerActivationProcess($email, $code){
        ...
        if($cek_email){
            ...
            $data = [
                'status' => 'email',
                'success' => false,
                'message' => 'Sorry, your email address '.$email.' was not found! Please contact admin or resend activation request.',
            ];
        }
