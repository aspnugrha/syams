            ...
            <div class="column-container bg-white w-100">
                <div class="collection-content p-4 m-0 m-md-5 text-dark">
                    @if ($data['success'])
                        ...
                        <form method="POST" id="form-data" class="d-flex flex-wrap gap-2 mb-2">
                            @csrf
                            ...
                            <button type="button" id="btn-create" onclick="createNewPassword()" class="btn btn-dark btn-lg text-uppercase w-100">Create New Password</button>
                        ...
                        {{-- <div class="d-flex justify-content-center">
                            <a href="{{ route('login') }}" class="btn btn-dark btn-lg text-uppercase w-100" style="width: 200px !important;">Login</a>
                        </div> --}}
                    @else
...
<script>
  ...
  function createNewPasswordProcess(){
    ...
    $.ajax({
        url: "{{ route('paneladmin.set-new-password.process') }}",
        type: "POST",
        data: postData,
        processData: false,
        contentType: false,
        ...
        success: function(res) {
            ...
            if(res.success){
              ...
              setTimeout(() => {
                window.location.href = "{{ route('paneladmin.login') }}"
              }, 1000);
            }else{
